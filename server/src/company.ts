...
export const companyRegister = async (req: CompanyRegisterRequest, res: Response, next: NextFunction) => {
  await catchAndLogError(
    ...
    async () => {
      ...
      const search = await db
        .select()
        .from(companyAccount)
        .where(eq(companyAccount.email, email));
  ...
  await catchAndLogError(
    ...
    async () => {
      ...
      if (search.length == 0) {
        ...
        if (!hashCheck) {
          throw new Error(`Failed to authenticate company with EMAIL=${email} due to invalid credentials`);
        }
